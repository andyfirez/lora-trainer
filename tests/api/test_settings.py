...
""",
    ...
    assert body["gpu_defaults"]["tf32"] is settings.gpu_defaults.tf32
    assert body["gpu_defaults"]["attention_mechanism"] == settings.gpu_defaults.attention_mechanism
...
@pytest.mark.asyncio
async def test_patch_gpu_defaults_persists(client, config_file, monkeypatch) -> None:
    monkeypatch.setattr(settings.gpu_defaults, "tf32", True)
    monkeypatch.setattr(settings.gpu_defaults, "attention_mechanism", "sdpa")

    response = await client.patch(
        "/settings/",
        json={"tf32": False, "attention_mechanism": "xformers"},
    )
    assert response.status_code == 200
    body = response.json()
    assert body["gpu_defaults"]["tf32"] is False
    assert body["gpu_defaults"]["attention_mechanism"] == "xformers"
    assert settings.gpu_defaults.tf32 is False
    assert settings.gpu_defaults.attention_mechanism == "xformers"

    saved = config_file.read_text(encoding="utf-8")
    assert "tf32 = false" in saved
    assert 'attention_mechanism = "xformers"' in saved
