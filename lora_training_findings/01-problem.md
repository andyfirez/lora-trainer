...
- С fix N + rank16/lr1e-3/warmup=0: **ep1–3 ok → ep4+ статик**; в шуме картинка почти не меняется (jul 2026)
...
| Winx_Bloom_CFTS | `datasets/Winx_Bloom` | 80 | — | 16/16 | 2 | 800 | стабилен | **ep1–3 ok → ep4+ статик** (fix N, lr1e-3, warmup=0, jul) |

Bloom — контрольный: другой персонаж, 6× данных, свежий cache. Fix N улучшил ранние эпохи (ep2–3 usable); likeness и поздняя стабильность — открыты (G).
...
Root cause likeness **не доказан**. После fix N (июль 2026):

1. **LR / warmup / overcooking (G)** — главный подозреваемый для ep4+; см. `08-fix-n-post-run-jul2026.md`
2. **Likeness** — ep2–3 post-N run приемлемы по качеству; likeness к датасету не закрыт
3. **Train/infer add_time_ids mismatch** — M
4. **Train loop audit** — D

Fix N (fp16 stack) **частично подтверждён** — убрал раннюю деградацию, недостаточен один.
