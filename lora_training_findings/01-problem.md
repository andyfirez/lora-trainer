...
- С H/I fix + rank16/lr1e-3: **полный шум** (реgression)
...
| Winx_Bloom_CFTS | `datasets/Winx_Bloom` | 80 | — | 16/16 | — | — | — | **шум** (rank16, lr1e-3, + H/I fix) |
...
Root cause **не доказан**. После code review (июнь 2026) наиболее вероятные направления:

1. **Effective LR ~20× ниже Kohya** (alpha/rank × lr) — гипотеза G (**не изолирована**, прогон смешан с H/I)
2. **Square crop без bucketing** — гипотеза C
3. **fp16 + AdamW8bit** vs bf16 + AdamW — гипотезы J/K
4. **Train loop audit** — гипотеза D

Детали: `03-hypotheses.md`, diff пайплайнов: `04-pipelines.md`.
