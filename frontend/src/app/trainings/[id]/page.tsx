...
export default function TrainingDetailPage({ params }: Props) {
  ...
  const handleRunJob = async () => {
    ...
    setRunError(null);
    try {
      const body: Parameters<typeof trainingsApi.createJob>[1] = { name: jobName.trim(), enqueue };
      const job = await trainingsApi.createJob(configId, body);
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-3">
        <Link href="/trainings" className="p-2 rounded-lg hover:bg-white/5 text-muted hover:text-text" aria-label="Back to trainings">
          <ArrowLeft size={18} />
        </Link>
      ...
      <Modal
        ...
        title="Run Job"
        description="Create a new training job from the current training config and optionally add it to the queue."
      >
