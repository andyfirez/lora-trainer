"""Training config router: CRUD + create job from training config."""
...
async def _get_training_config(config_id: int, service: JobConfigServiceDep):
    try:
...
async def create_training(
    body: TrainingConfigCreate, service: JobConfigServiceDep
) -> JobConfigResponse:
...
async def get_training(config_id: int, service: JobConfigServiceDep) -> JobConfigResponse:
    config = await _get_training_config(config_id, service)
    return JobConfigResponse.model_validate(config, from_attributes=True)
...
async def update_training(
    ...
    await _get_training_config(config_id, service)
