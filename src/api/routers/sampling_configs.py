"""Sampling configs router: CRUD + create job from sampling config."""
...
async def _require_sampling_config(config_id: int, service: JobConfigServiceDep):
...
async def get_sampling_config(config_id: int, service: JobConfigServiceDep) -> JobConfigResponse:
    config = await _require_sampling_config(config_id, service)
    return JobConfigResponse.model_validate(config, from_attributes=True)
...
async def update_sampling_config(
    ...
    await _require_sampling_config(config_id, service)
...
async def create_job_from_sampling_config(
    ...
    await _require_sampling_config(config_id, config_service)
