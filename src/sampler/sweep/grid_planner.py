...
from src.sampler.sweep.models import SWEEP_PARAM_ORDER, GridLayout, SweepCombination, SweepParameters
...
def plan_grids(parameters: SweepParameters, grid: GridLayout) -> list[GridPlan]:
    ...
    if len(vary_keys) == 1:
        ...
        if not combos:
            return []
        values = parameters.get_param(key).effective_values()
...
def _build_single_grid(
    ...
    x_values = parameters.get_param(x_axis).effective_values() if x_axis in parameters.vary_keys_with_values() else []
    y_values = parameters.get_param(y_axis).effective_values() if y_axis in parameters.vary_keys_with_values() else []

    if x_axis == y_axis:
        x_values = x_values or parameters.get_param(x_axis).effective_values()
        y_values = [None]
    elif not x_values:
        x_values = [None]
    if not y_values:
